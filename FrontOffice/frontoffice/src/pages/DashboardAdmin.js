...
// sections
import { AppTasks, AppNewsUpdate, AppOrderTimeline, AppCurrentVisits, AppWebsiteVisits, AppTrafficBySite, AppWidgetSummary, AppCurrentSubject, AppConversionRates, } from '../sections/@dashboard/app';
// ----------------------------------------------------------------------
...
function getVilleC(){
            ...
            villes.map((row,index)=>{
                v.push({label: row.nom, value: ctrVilles.filter((ele) => ele.refVille === row.code).length});
            })
...
function getCentreRes() {
  ...
  function getData (id){
    ...
    console.log("data",reservation.filter((ele) => ele.dateResv.substring(0, 10) === '2023-05-18' && ele.refService===id))
   return res
  }
