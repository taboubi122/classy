...
export default function ServicesPage() {
	...
	const [refselectedCentre, setRefselectedCentre] = useState([]);
    ...
    const [errMessageCtr, setErrMessageCtr]=useState('');
    const [errorCtr, setErrorCtr] = useState(false);
    ...
    if(refselectedCentre.length===0){
      setErrorCtr(true)
      setErrMessageCtr('Le champ est obligatoire.')
    }else{
      setErrorCtr(false)
      setErrMessageCtr('')
      setTest6(false)
    ...
    if(!test1 && !test2 && !test3 && !test4 && !test5 && !test6){
      window.location.reload();
		axios.post(`http://localhost:5000/api/addService`, {
      nom
			, description
			, prix
			, duree
  ...
  const [errMessageNomUpdate, setErrMessageNomUpdate]=useState('');
    ...
    const [errMessageCtrUpdate, setErrMessageCtrUpdate]=useState('');
    const [errorCtrUpdate, setErrorCtrUpdate] = useState(false);
	...
	function onUpdate(id, nomUpdate, descriptionUpdate, prixUpdate, dureeUpdate, refselectedCentre, refselectedCateg) {
    if(nomUpdate==='' && descriptionUpdate==='' && prixUpdate.length===0 && dureeUpdate==='' && refselectedCentre.length===0 && refselectedCateg.length===0){
      setErrorNomUpdate(true)
      console.log("nom: ",errorNomUpdate)
      setErrorDescUpdate(true)
      console.log("desc: ",errorDescUpdate)
      setErrorDureeUpdate(true)
      console.log("duree: ",errorDureeUpdate)
      setErrorPrixUpdate(true)
      console.log("prix: ",errorPrixUpdate)
      setErrorCategUpdate(true)
      console.log("categ: ",errorCategUpdate)
      setErrorCtrUpdate(true)
      console.log("ctr: ", errorCtrUpdate)
      setErrMessageCtrUpdate('Doit modifier un champ au minimum.')
    ...
    if(nomUpdate.length>100 && !nomUpdate===''){
      ...
      setErrorCtrUpdate(false)
      setErrMessageCtrUpdate('')
    ...
    if(!test1Update && !test2Update && !test3Update && !test4Update && !test5Update && !test6Update ){
      window.location.reload();
      axios.put(`http://localhost:5000/api/UpdateServices/${id}`, {
				nomUpdate
				, descriptionUpdate
				, prixUpdate
				, dureeUpdate
              ...
              <div className="popup">
                ...
                <Stack spacing={3}>
                  ...
                  <Select
                    ...
                    <InputLabel id="my-select-label">Centre *</InputLabel>
                  <Select
                    labelId="my-select-label"
                    value={refselectedCentre}
                    onChange={(event) => {
                        setRefselectedCentre(event.target.value);
                      }}
                    error={errorCtr}
                    >
                    {centre.map((opt) => (
                    <MenuItem key={opt.reference} value={opt.reference}>
                    {opt.nom}
                    </MenuItem>
                ))}
                    </Select>
                    {errorCtr? <span className="err">{errMessageCtr}</span>:''}
                  <TextField
                    name="description"
                    label="Description *"
                    value={description}
              ...
              <div className="popup">
                ...
                <Stack spacing={3}>
                  ...
                  <Select
                    ...
                    {errorCtrUpdate? <span className="err">{errMessageCtrUpdate}</span>:''}
                  <TextField
                    name="description"
                    label="Description"
                ...
                <Button
                  ...
                  variant="contained"
                  onClick={() =>
                    onUpdate( referenceUpdate, nomUpdate, descriptionUpdate, prixUpdate, dureeUpdate, refselectedCentre, refselectedCateg )
                  }
                >
                  Mise à jour
                </Button>
