...
export default function HorairePerso() {
	const params = useParams();
	const CIN = params.id;
	useEffect(() => {
		axios
			.get(`http://localhost:5000/api/getHorairePerso/${CIN}`)
	...
	function onUpdate(referenceUpdate, ouvertureUpdate, fermetureUpdate, refselectedJourUpdate) {
		...
		if(ouvertureUpdate==='' && fermetureUpdate===''){
		  ...
		  if(!test1 && !test2){
		    ...
		    axios.put(`http://localhost:5000/api/UpdateHorairePerso/${referenceUpdate}`, {
				ouvertureUpdate
				, fermetureUpdate
    ...
    const [isActiveList, setIsActiveList] = useState(filteredUsers.map(() => false));

    const toggleActive = (index) => {
        setIsActiveList(prevState => {
  ...
  const isActive = isActiveList[index];
  ...
  return (
    <TableRow
      ...
      tabIndex={-1}
      role="checkbox"
      selected={false} // Assuming you don't need a selectedUser property
    >
      ...
      <TableCell align="left" width="120">  
        <IconButton
          size="large"
          sx={{ color: isActive ? "black" : "success.main" }}
